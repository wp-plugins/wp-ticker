<?php
  $istgespeichert_w = "&Auml;nderungen wurden gespeichert.";
  $allgemeines_w = "Allgemeines";
  $deinstall_w = "Deinstallieren";
  $deinstall_hinweis_w = "l&ouml;scht nach Deaktivierung alle zum Plugin geh&ouml;renden Einstellungen";

  $codegenerator_w = "Ticker-Einstellungen";
  $tickerid_w = "Ticker-ID";
  $tickerid_info_w = "Integer, muss eindeutig sein!";
  $tickersrc_w = "Datenquelle";
  $tickersrc_db_w = "Datenbank";
  $tickersrc_own_w = "eigener Text";
  $tickersrc_rss_w = "RSS-Feed";
  $duration_w = "Dauer f&uuml;r";
  $tickershowtime_w = "Anzeige";
  $tickershowtime_info_w = "ms";
  $tickerintime_w = "Einblendung";
  $tickerintime_info_w = "ms";
  $tickerouttime_w = "Ausblendung";
  $tickerouttime_info_w = "ms";

  $tickerreloadtime_w = "Reaload-Intervall";
  $tickerreloadtime_info_w = "Min. (0 oder leer: kein auto. Reload)";
  $tickerreloadpausetime_w = "Reload-Pause";
  $tickerreloadpausetime_info_w = "s (0 oder leer: keine Pause)";

  $tickertype_w = "Tickertyp";
  $data_txt_db = "Kategorie(n) w&auml;hlen";
  $data_txt_own = "Eigene Texte";
  $data_txt_rss = "Feed-URLs eingeben (mit Return getrennt)";
  $tickermaxitems_w = "Max. Eintr&auml;ge";
  $tickermaxitems_info_w = "leer: alle zeigen";
  $tickermaxchars_w = "Max. Zeichen";
  $tickermaxchars_info_w = "leer: alles zeigen";
  $template_w = "Template";
  $template_head_w = "Variable f&uuml;r &Uuml;berschrift";
  $template_content_w = "Variable f&uuml;r Kontent";
  $memo_w = "Memo";



  $own_ticker_delete_w = "Wollen Sie diesen Ticker-Text loeschen: ID=";
  $own_ticker_startdata_w = "Start-Datum";
  $own_ticker_enddata_w = "End-Datum";
  $own_ticker_autodel_w = "auto. L&ouml;schen";
  $own_ticker_neu_w = "Neu";
  $own_ticker_texthinweis = "Ihr Text";

  $tag_w = "Tag";
  $monat_w = "Monat";
  $jahr_w = "Jahr";

  $ticker_head_w = "Angelegte Ticker";

  $editbtn_w = "Bearbeiten";
  $deletebtn_w = "L&ouml;schen";
  $codebtn_w = "Code";
  $createtickerbtn_w = "Ticker anlegen";

  $delete_quest_w = "Wollen Sie diesen Ticker loeschen: ID=";
  $code_info_w = "Binden Sie folgenden Code ein:\\n";

  $speichern_w = "Speichern";
  $abbruch_w = "Abbrechen";

  $fußnote_w = "Neue Ticker-Module und Updates werden unter <a href='http://www.stegasoft.de/' target='_blank'>www.stegasoft.de</a> ver&ouml;ffentlicht.";


?>